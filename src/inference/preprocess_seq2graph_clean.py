...
class TestDataset(Dataset):
    ...
    def correct_masks(self, min_cell_size):
        ...
        for ind_data in range(self.__len__()):
            ...
            if not np.all(np.unique(result) == np.unique(res_save)):
                warnings.warn(
                    f"pay attention! the labels have changed from "
                    f"{np.unique(res_save)} to {np.unique(result)}")
                ...
                while True:
                    ...
                    if np.any(counts < min_cell_size):
                        ...
                        if first_label_ind.size > 1:
                            first_label_ind = first_label_ind.squeeze()[0]
                        first_label_num = un_labels[first_label_ind]
                        curr_mask = np.logical_and(result == id_res,
                                                   re_label_mask == first_label_num)
            ...
            if per_cell_change or per_mask_change:
                ...
                print(f"per_mask_change={per_mask_change}, per_cell_change={per_cell_change}, "
                      f"number of changed pixels: {n_pixels}")
    ...
    def preprocess_features_loop_by_results_w_metric_learning(self, path_to_write, dict_path):
        ...
        self.flag_new_roi = self.global_delta_row > self.roi_model['row'] \
            or self.global_delta_col > self.roi_model['col']
        ...
        if self.flag_new_roi:
            ...
            print(f"old ROI: {self.roi_model}, "
                  f"new: row: {self.global_delta_row}, col: {self.global_delta_col}")
        else:
        ...
        for ind_data in range(self.__len__()):
            ...
            for ind, id_res in enumerate(np.unique(result)):
                ...
                embedded_feat = self.extract_freature_metric_learning(
                    properties.bbox, img.copy(), result.copy(), id_res)
                ...
                df.loc[row_ind, "max_intensity"], df.loc[row_ind, "mean_intensity"], \
                df.loc[row_ind, "min_intensity"] = \
...
def create_csv(input_images, input_seg, input_model, output_csv, min_cell_size):
    ...
    ds.preprocess_features_loop_by_results_w_metric_learning(
        path_to_write=path_output, dict_path=dict_path)
